...
namespace easyfis.Reports
{
    public class RepCollectionSummaryReportController : Controller
    {
        ...
        public ActionResult CollectionSummaryReport(String StartDate, String EndDate, String CompanyId, String BranchId)
        {
            ...
            headerPage.AddCell(new PdfPCell(new Phrase("Collection Summary Report", fontArial17Bold)) { Border = 0, HorizontalAlignment = 2 });
            ...
            var collections = from d in db.TrnCollections
                              where d.ORDate >= Convert.ToDateTime(StartDate)
            ...
            if (collections.Any())
            {
                ...
                float[] widthsCellsData = new float[] { 15f, 13f, 25f, 25f, 20f };
                ...
                Decimal total = 0;
                foreach (var collectionInvoice in collections)
                {
                    data.AddCell(new PdfPCell(new Phrase(collectionInvoice.ORNumber, fontArial10)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 5f, PaddingLeft = 5f });
                    data.AddCell(new PdfPCell(new Phrase(collectionInvoice.ORDate, fontArial10)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 5f, PaddingLeft = 5f });
                    data.AddCell(new PdfPCell(new Phrase(collectionInvoice.Customer, fontArial10)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 5f, PaddingLeft = 5f });
                    data.AddCell(new PdfPCell(new Phrase(collectionInvoice.Particulars, fontArial10)) { HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 5f, PaddingLeft = 5f });
                    data.AddCell(new PdfPCell(new Phrase(collectionInvoice.Amount.ToString("#,##0.00"), fontArial10)) { HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 5f, PaddingLeft = 5f });
                    total += collectionInvoice.Amount;
                }

                // =====
                // Total
                // =====
                data.AddCell(new PdfPCell(new Phrase("Total", fontArial10Bold)) { Colspan = 4, HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 5f, PaddingRight = 10f, PaddingLeft = 10f });
